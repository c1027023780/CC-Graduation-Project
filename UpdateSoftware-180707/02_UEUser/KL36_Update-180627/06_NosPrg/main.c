...
int main(void)
{
    ...
    uecom_switch(UECOM_ON);    //给通信模组供电（因需延时，所以最先操作）
    ...
    while(1)
    {
		...
		uecom_switch(UECOM_REBOOT);    //通信模组重启
    }
    //printf("AHL Initialize Successfully\n");
    LCD_ShowString(4,300,BLUE,GRAY,"AHL Initialize Successfully ");
    //（7.4）【根据实际需要增删】获得LCD需要显示的数据并显示
    //（7.4.1）显示型号
	strncpy(mString,gFlashData.equipType,20);
	mString[20]=0;
	LCD_ShowString(70,45,BLUE,GRAY,mString);
	//（7.4.2）显示软件版本
	strncpy(mString,gFlashData.softVer,4);
	mString[4]=0;
	LCD_ShowString(118,65,BLUE,GRAY,mString);
    //(7.4.3)显示设备的IMSI号
    uecom_getIMSI(imsi+1);    //获取IMSI号
    imsi[0]=' ';   //第一个字符为空格，占位
    imsi[16]=0;    //字符串的结束符
    //printf("当前套片IMSI号为：%s\n",imsi+1);  //串口输出IMSI号
    ...
    for(mi=0;mi<5;mi++)
    {
	    if(uecom_getCurCommunity(mString))
		    continue;
	    mString[17] = 0;
	    LCD_ShowString(89,129,BLUE,GRAY,mString);
	    break;
    }
    //（7.4.7）显示设备的信号强度
    for(mi=0;mi<5;mi++)
   ...
   // printf("通信模组信号强度：%d\n",mj);
    LCD_ShowString(105,150,BLUE,GRAY,(uint_8 *)myItoa(mj,mString,10));
	//（7.4.8）显示IP
	strncpy(mString,gFlashData.serverIP,15);
	mString[15]=0;
	LCD_ShowString(65,169,BLUE,GRAY,mString);
    //（7.4.9）显示PORT
	strncpy(mString,gFlashData.serverPort,5);
	mString[5]=0;
	...
	LCD_ShowString(180,210,BLUE,GRAY,
			(uint_8 *)myItoa(gFlashData.sendFrequencySec,mString,10));
    //测试http使用，1AC硬件支持
...
//            LCD_ShowString(4,300,BLUE,GRAY,"提示H1-1002，初始化成功");
            ...
            LCD_ShowString(105,150,BLUE,GRAY,(uint_8 *)myItoa(gUserData.signalPower,mString,10));
	    ...
	    switch(mflag)
	    {
            ...
            uecom_init(mRetdata,gFlashData.serverIP,gFlashData.serverPort);//重新初始化
            goto main_loop;
        main_loop:
            mSendFlag = 0;         //修改发送标记
        }
        ...
        {
        	mRecvCount++;
        	LCD_ShowString(90,249,BLUE,GRAY,myItoa(mRecvCount,mString,10));
        	LCD_ShowString(4,300,BLUE,GRAY,"AHL Recv one frame                 ");
        	mflag = 0xff;
        	mSendLen = 0;
        	memcpy(mCmd,gRecvBuf,2);
...
void LCD_ShowInit()
{
	...
	LCD_ShowString(4,209,BLACK,GRAY,"TSI:     Fre(s):   ");
	...
	LCD_ShowString(4,276,BLACK,GRAY,"State:");
