...
int main(void)
{
    ...
    imsi[16]=0;    //字符串的结束符
    //printf("当前套片IMSI号为：%s\n",imsi+1);  //串口输出IMSI号
    LCD_ShowString(70,85,BLUE,GRAY,imsi+1);
    //（7.4.5）显示设备的温度
    ...
    for(mi=0;mi<5;mi++)
    {
	    if(uecom_getCurCommunity(mString))
		    continue;
	    mString[17] = 0;
	    LCD_ShowString(90,129,BLUE,GRAY,mString);
	    break;
    }
    //（7.4.7）显示设备的信号强度
    for(mi=0;mi<5;mi++)
