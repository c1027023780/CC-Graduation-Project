...
版本更新：V1.0,2017.8.16；V2.2，2018.1.6（WYH)；V3.0,2018.6.21
=================================================================
...
20180523 wb
（1）重新规范了uecom构件，更改了对外的接口。
（2）新增设置调试模式。
（3）优化了执行流程，改变了解析过程，提高了初始化速度
（4）延时函数更改为外部设置。在有操作系统时，可以使用不占用mcu的延时方式，提高了适用性。

20180525 wb
（1）新增了gnss功能。
（2）uecom_sendCmd中的超时等待，采用递增方式已适应不同网络环境。
（3）新增30秒测试一次gnss信息，并通过串口将信息打出。
（4）去掉了35秒关闭电源，为了测试出GNSS信息。

20180612 wb
新增命令获取指定信息的方式：
上位机传来的命令信息格式为：命令码（1个字节）+数据（可为空）
命令含义：
0x30：读取flash中保存的所有信息
0x31：读取产品名称
0x32：读取产品序列号
0x33：读取产品型号
0x34：读取生产厂家
0x35：读取软件版本
0x36：读取生产时间（64位时间戳，可转为年月日时分秒）
0x37：读取用户名
0x38：读取用户手机号
0x39：读取服务器IP
0x3a：读取服务器端口号
...
“B2”：更改flash中的服务器信息
“B3”：更改用户存入flash的信息
（2）重新规范了程序，删除冗余代码，提高健壮性。
（3）更改初始化失败的错误提示。
当前提示及含义：
“F1-1001”：与通信模块通信失败
“F1-1002”：SIM卡读取失败
“F1-1003”：联网失败或连接服务器失败
（4）增加了LCD上的显示内容，包括发送频率和接收到的数据帧数。
（5）为了100-1AA与101-1AC的主函数统一，暂时注释了获取GNSS的代码。
=============================================================================
声明：
